...
def reorder_episodes_by_start(episodes):
    """
    مرتب‌سازی اپیزودها بر اساس start و اختصاص episode_id جدید از 1
    """
    # مرتب‌سازی اپیزودها بر اساس start
    sorted_episodes = sorted(episodes, key=lambda x: x['start'])
    
    # ایجاد لیست جدید با episode_idهای به‌روزشده
    result = []
    for new_id, episode in enumerate(sorted_episodes, 1):
        new_episode = episode.copy()
        new_episode['episode_id'] = new_id
        result.append(new_episode)
...
def extract_episodes(csv_path):
    ...
    return reorder_episodes_by_start(episodes)
    

def extract_episodes_x(csv_path):
    episodes = []
    data = []
    data_text = []
    ...
    with open(csv_path, newline='', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            row['End'] = float(row['End']) if row['End'] else None
            data.append(row)
            data_text.append(row['Home phase']+' ** '+row['Away phase'])
    ...
    for row in data:
        ...
        if eid not in grouped:
            grouped[eid] = [];
        if eid not in grouped_text:
            grouped_text[eid] = [];
            
        grouped[eid].append(row)
        grouped_text[eid].append(data_text[px])
        px+=1;
...
def afteroverlap(out_episode_path,fps=5):
    for epi in np.sort(glob.glob(out_episode_path+'/episode_*/')):
        epilast=[epi][0]
        nparts = len(glob.glob(epi+'part*/'))
        
        if nparts==1:
            frames_folder = glob.glob(epi+'part*')[0]
            _recreate_video_from_frames(frames_folder, fps=fps, verbose=False)
        else:  
            for path in np.sort(glob.glob(epi+'part*/*.jpg')):
                if 'part_0001' not in path:
                    des = epilast.item()+'part_0001/'+path.split('/')[-1]
                    if os.path.exists(des)==False:
                        os.system('mv '+path.item() +' '+des)

            if 'part_0001' not in path:
                folderpart = '/'.join(path.item().split('/')[:-1])
                os.system('rm -rf ' + folderpart)
    ...
    os.system('rm -rf ' + out_episode_path+'/episode_*/part_0003')
    os.system('rm -rf ' + out_episode_path+'/episode_*/part_0004')
    os.system('rm -rf ' + out_episode_path+'/episode_*/part_0005')
    os.system('rm -rf ' + out_episode_path+'/episode_*/part_0006')
    os.system('rm -rf ' + out_episode_path+'/episode_*/part_0007')
    os.system('rm -rf ' + out_episode_path+'/episode_*/part_0008')
